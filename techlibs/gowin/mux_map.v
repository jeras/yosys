...
module \$_MUX_ (input A, B, input S, output Y);
	\$lut #(.INIT(16'b1100_1010)) lut (.A({S, B, A}), .Y(Y));
endmodule

module \$_MUX4_ (input A, B, C, D, input S, T, output Y);
	wire y0, y1;
	\$_MUX_ mux2_0 (.A(A), .B(B), .S(S), .O(y0));
	\$_MUX_ mux2_1 (.A(C), .B(D), .S(S), .O(y1));
	MUX2_LUT5 mux2_mux4 (.O(Y), .I0(y0), .I1(y1), .S0(T));
endmodule

module \$_MUX8_ (input A, B, C, D, E, F, G, H, input S, T, U, output Y);
	wire y0, y1;
	\$_MUX4_ _TECHMAP_REPLACE_mux4_0 (.A(A), .B(B), .C(C), .D(D), .S(S), .T(T), .O(y0));
	\$_MUX4_ _TECHMAP_REPLACE_mux4_1 (.A(E), .B(F), .C(G), .D(H), .S(S), .T(T), .O(y1));
...
module \$_MUX16_ (input A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P, input S, T, U, V, output Y);
	wire y0, y1;
	\$_MUX8_ _TECHMAP_REPLACE_mux8_0 (.A(A), .B(B), .C(C), .D(D), .E(E), .F(F), .G(G), .H(H), .S(S), .T(T), .U(U), .O(y0));
	\$_MUX8_ _TECHMAP_REPLACE_mux8_1 (.A(I), .B(J), .C(K), .D(L), .E(M), .F(N), .G(O), .H(P), .S(S), .T(T), .U(U), .O(y1));
...
module \$_MUX32_ (input A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P, QA, QB, QC, QD, QE, QF, QG, QH, QI, QJ, QK, QL, QM, QN, QO, QP, input S, T, U, V, W, output Y);
	wire y0, y1;
	\$_MUX16_ _TECHMAP_REPLACE_mux16_0 (.A( A), .B( B), .C( C), .D( D), .E( E), .F( F), .G( G), .H( H), .I( I), .J( J), .K( K), .L( L), .M( M), .N( N), .O( O), .P( P), .S(S), .T(T), .U(U), .U(V), .O(y0));
	\$_MUX16_ _TECHMAP_REPLACE_mux16_1 (.A(QA), .B(QB), .C(QC), .D(QD), .E(QE), .F(QF), .G(QG), .H(QH), .I(QI), .J(QJ), .K(QK), .L(QL), .M(QM), .N(QN), .O(QO), .P(QP), .S(S), .T(T), .U(U), .U(V), .O(y1));
